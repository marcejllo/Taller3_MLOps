...
with mlflow.start_run(run_name="GradientBoosting_Optimization"):
    study_gb.optimize(objective_gb, n_trials=10, show_progress_bar=True)
    ...
    final_r2 = r2_score(y_test, y_pred_final_gb)
    final_mae = mean_absolute_error(y_test, y_pred_final_gb)
    final_rmse = mean_squared_error(y_test, y_pred_final_gb)**0.5
    ...
    signature = infer_signature(X_train, final_model_gb.predict(X_train))

    mlflow.sklearn.log_model(
        sk_model=final_model_gb,
        artifact_path="model_gradient_boosting",
        input_example=input_example,
        signature=signature
    )
    
    opt_history_path = "optimization_history_gb.png"
    opt_slice_path = "slice_plot_gb.png"
    
    vis.plot_optimization_history(study_gb).write_image(opt_history_path)
    vis.plot_slice(study_gb).write_image(opt_slice_path)
